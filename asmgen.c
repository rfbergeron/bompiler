...
static const char FUNCTION_PROLOG[] = "PUSH rbp\n"
                               "MOV rbp, rsp\n"
                               "PUSH r12\n"
                               "PUSH r13\n"
                               "PUSH r14\n"
                               "PUSH r15\n";
static const char FUNCTION_EPILOG[] = "POP r15\n"
                               "POP r14\n"
                               "POP r13\n"
                               "POP r12\n"
                               "MOV rsp, rbp\n"
...
int assign_space (ASTree *tree) {
    SymbolValue *symval = map_get(current_symtable, (void *) tree->lexinfo,
            strlen(tree->lexinfo));
    size_t required_padding = tree->type.alignment -
        (stack_window % tree->type.alignment);
...
int assign_vreg(TypeSpec *type, InstructionData *data, const size_t vreg_num,
    ...
    char *dest = opflags & SOURCE_OPERAND ? data->src_operand : data->dest_operand;
    char reg_width = 0;
...
int resolve_object (ASTree *ident, InstructionData *out, unsigned int flags) {
    ...
    SymbolValue *symval = map_get(current_symtable, (void *) ident->lexinfo,
...
int translate_ident(ASTree *ident, InstructionData *data, unsigned int flags) {
    if(flags & WANT_OBJ_VADDR) {
        ...
        assign_vreg(&(ident->type), data, vreg_count++, DEST_OPERAND & WANT_OBJ_VADDR);
    } else {
...
int translate_unop(ASTree *operator, InstructionData *data, InstructionEnum num,
        unsigned int flags) {
...
int translate_binop(ASTree *operator, InstructionData *data, InstructionEnum num,
        unsigned int flags) {
    data->instruction = instructions[num];
...
int write_text_section() {
    return 0;
}

int write_data_section(struct llist *global_var_stack) {
    return 0;
}

int write_bss_section (struct llist *uninitialized_data_stack) {
    return 0;
}
